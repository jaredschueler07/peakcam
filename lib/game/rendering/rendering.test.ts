...
import { rampRise, RAMP_MAX_RISE, sagAt, WorldRenderer } from "./WorldRenderer";
...
/** A two-point run whose single segment drops `dropM` over `runM` — a simplified cliff tail. */
function syntheticCourse(runM: number, dropM: number): { world: SimulationWorld; run: RealRun } {
  const points = [{ x: 0, y: 2000, z: 0 }, { x: 0, y: 2000 - dropM, z: runM }];
  const run: RealRun = {
    kind: "real", sourceIndex: 0, name: "Cliff", difficulty: null, halfWidthM: 18,
    points, lengthM: runM, finishM: runM, gates: [],
    ramps: [{ key: 0, distanceM: runM - 45, x: 0, y: 2000 - dropM * (runM - 45) / runM, z: runM - 45, heading: 0 }],
  };
  const terrain: TerrainSampler = {
    kind: "real", profile, seed: 1, noiseOffset: { x: 0, z: 0 }, realRuns: [run], mainLift: null,
    height(_x, z) { return 2000 - dropM * Math.max(0, Math.min(runM, z)) / runM; },
    normal(_x, _z, out) { out.x = 0; out.y = 1; out.z = 0; return out; },
    trailField() { return 0; },
    nearestTrail(_x, _z, out) { return out; },
  };
...
test("a cliff-tail ramp renders as a ramp, not a beam hanging over the course", () => {
  ...
  const { world } = syntheticCourse(800, 700);
  const state = createSimulation(profile, profile.seed);
  const scene = new THREE.Scene();
  const renderer = new WorldRenderer(scene, profile, world);
  renderer.update(state, 0);

  const ramp = scene.children.find((child) => child instanceof THREE.Group && child.visible &&
    (child.userData as { rail?: THREE.Mesh }).rail) as THREE.Group;
  assert.ok(ramp, "the tail ramp is placed");
  const { rail, rail2, banner } = ramp.userData as Record<string, THREE.Mesh>;
  ...
  // The banner sits at the ramp entry, where the rails' uphill ends land whatever the rise is.
  assert.deepEqual(banner.position.toArray(), [0, 4.2, -1.5]);
});

test("a gentle course leaves the ramp pitch exactly as the terrain dictates", () => {
  const { world } = syntheticCourse(800, 80);
  const state = createSimulation(profile, profile.seed);
  const scene = new THREE.Scene();
  const renderer = new WorldRenderer(scene, profile, world);
  renderer.update(state, 0);
  const ramp = scene.children.find((child) => child instanceof THREE.Group && child.visible &&
    (child.userData as { rail?: THREE.Mesh }).rail) as THREE.Group;
  const rail = (ramp.userData as Record<string, THREE.Mesh>).rail;
