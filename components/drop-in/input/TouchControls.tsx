...
export default function TouchControls({ adapter }: { adapter: TouchAdapter }) {
  return (
    <div className="pointer-events-none absolute inset-0 z-20 touch-none sm:hidden" aria-label="Touch controls">
      <div className="absolute bottom-[max(1rem,env(safe-area-inset-bottom))] left-[max(1rem,env(safe-area-inset-left))] grid h-28 w-36 place-items-center rounded-full border border-cream-50/70 bg-ink/20 font-mono text-[9px] uppercase tracking-wide text-cream-50" aria-hidden>Drag to carve</div>
