...
import { trackResortView, trackCamClick, trackShareClick } from "@/lib/posthog";
...
function ShareButton({ resortSlug, resortName }: { resortSlug: string; resortName: string }) {
  ...
  async function handleShare() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: `${resortName} — PeakCam`, url });
        trackShareClick(resortSlug, "native");
      } catch {
        // user cancelled share sheet — not an error
      }
    } else {
      await navigator.clipboard.writeText(url);
      trackShareClick(resortSlug, "copy");
      setCopied(true);
  ...
  return (
    <button
      ...
      className="text-text-muted hover:text-cyan text-sm border border-border hover:border-border-hi
                 rounded-lg px-3 py-1.5 transition-all duration-150 flex items-center gap-1.5"
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <ShareButton resortSlug={resort.slug} resortName={resort.name} />
              </div>
            </div>
          </div>
        </div>
