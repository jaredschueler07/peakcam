...
test('forest placement never edits source heights, regardless of seed or corridor type',()=>{
 const copy=field.heights.slice();
 for (const seed of [1,2,19]) {
   const output=bakeMountainDetail(field,structuredClone(trails),seed);
   assert.deepEqual(output,copy);assert.deepEqual(field.heights,copy);
 }
});
test('source normals match the same interpolated physical height',()=>{
...
test('overlapping trail corridors never reshape the DEM',()=>{
...
test('tree placement respects explicit DEM treeline and avoid trail corridors',()=>{
