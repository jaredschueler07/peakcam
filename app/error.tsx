 ...
 * Root error boundary.
 *
 * Any Server Component that throws (most often a Supabase read that hit the
 * anon role's statement timeout during a sync window) lands here instead of
 * Next's bare "Application error" page — which on Vercel also logs a
 * "Failed to load static file for page: /500" error because the App Router
 * has no pages/500.html to fall back to.
 */
export default function RootError({
  ...
  useEffect(() => {
    console.error("[PeakCam] page failed to render:", error);
  }, [error]);
    ...
    <div className="min-h-screen pc-paper flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-lg rounded-[18px] border-[1.5px] border-ink bg-cream-50 p-7 shadow-stamp md:p-9">
        ...
        <h1 className="font-display font-black text-ink text-3xl leading-[1.02] tracking-[-0.02em] md:text-4xl">
          We couldn&rsquo;t load this <em className="text-alpen italic font-bold">page</em>.
        </h1>

        <p className="mt-3 text-[14px] leading-relaxed text-bark">
          Snow reports and cams come from live feeds, so this is usually temporary — give it
          another go.
        </p>
